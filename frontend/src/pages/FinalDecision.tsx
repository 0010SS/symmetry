...
import data from "@/data/final-decision.json";

// ---------- types (relaxed to match your JSON) ----------
type Strategy = {
  direction?: string;
  entry?: { rule?: string; band?: string; conditions?: string[] };
  stop?: { rule?: string; level?: string; invalidation?: string };
  targets?: string[] | number[];
  sizing?: { max_size_pct_portfolio?: number; risk_per_trade_pct?: number };
  one_liner?: string;
};

type StrategyMatrix = {
  aggressive?: { annual?: Strategy; swing?: Strategy; intraday?: Strategy };
  neutral?: { annual?: Strategy; swing?: Strategy; intraday?: Strategy };
  conservative?: { annual?: Strategy; swing?: Strategy; intraday?: Strategy };
};
...
type FinalDecisionData = {
  ...
  risk_budget_summary?: {
    exposure_after_trade?: string;
    position_risk?: string;
    portfolio_risk_considerations?: string[];
...
export default function FinalDecision() {
  ...
  // If you don’t actually load async, you can keep these as constants:
  const [loading] = useState(false);
  const [err] = useState<string | null>(null);

  const [activeTab, setActiveTab] = useState<
    "overview" | "matrix" | "execution" | "risk" | "monitoring"
  >("overview");

  const riskData = (data as FinalDecisionData) ?? {};
                  ...
                  return (
                    <div key={timeframe} className="border rounded-lg p-4">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <Badge
                          className={getDirectionColor(strategy.direction)}
                        >
                      ...
                      <div className="space-y-3 text-sm">
                        ...
                        {strategy.sizing && (
                          <div>
                            ...
                            <span className="text-muted-foreground">
                              {(
                                strategy.sizing.max_size_pct_portfolio ?? "—"
                              ).toString()}
                              %{" / "}
                              {(
                                strategy.sizing.risk_per_trade_pct ?? "—"
                              ).toString()}
                              %
                            </span>
                          </div>
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center gap-4 mb-8">
          ...
          <h1 className="text-3xl font-bold">
            {riskData?.meta?.pageTitle || "Final Investment Decision"}
          </h1>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <div className="text-center">
                <div className="text-4xl font-bold text-yellow-600 mb-2">
                  {String(
                    (riskData.summary_cards?.action || riskData.decision) ?? "—"
                  ).toUpperCase()}
                </div>
                <p className="text-sm text-muted-foreground">
                  Recommended Action
            ...
            <Card className="trading-panel-enhanced">
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  {Object.entries(riskData.strategy_matrix || {}).flatMap(
                    ([profile, strategies]) =>
                      Object.entries(
                        (strategies as Record<string, Strategy>) || {}
                      ).map(([timeframe, strategy]) => (
                        <div
                          key={`${profile}-${timeframe}`}
                          className="grid grid-cols-1 lg:grid-cols-3 gap-4 p-3 border rounded"
                        >
                          ...
                          <div>
                            <Badge
                              className={getDirectionColor(
                                (strategy as Strategy)?.direction
                              )}
                            >
                              {String(
                                (strategy as Strategy)?.direction || ""
                          ...
                          <div className="text-sm text-muted-foreground">
                            Entry:{" "}
                            {(strategy as Strategy)?.entry?.band || "N/A"} |{" "}
                            Stop: {(strategy as Strategy)?.stop?.level || "N/A"}{" "}
                            | Size:{" "}
                            {(
                              (strategy as Strategy)?.sizing
                                ?.max_size_pct_portfolio ?? "—"
                            ).toString()}
                            %
                          </div>
                        </div>
            ...
            <Card className="trading-panel-enhanced">
              ...
              <CardContent>
                <div className="space-y-3">
                  {(riskData.conflicts_and_resolutions || []).map(
                    (conflict, index) => (
                      <div
                        key={index}
                        className="p-3 border rounded-lg bg-amber-50/50"
                      >
        ...
        {activeTab === "matrix" && <div>{renderStrategyMatrix()}</div>}
        ...
        {activeTab === "execution" && (
          <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-4">
                    <div>
                      ...
                      <span className="text-muted-foreground">
                        {riskData.execution_ticket?.instrument || "—"}
                      </span>
                    </div>
                    <div>
                      <span className="font-medium">Side: </span>
                      <span className="text-muted-foreground">
                        {riskData.execution_ticket?.side || "—"}
                      </span>
                    </div>
                    <div>
                      <span className="font-medium">Entry Method: </span>
                      <span className="text-muted-foreground">
                        {riskData.execution_ticket?.entry_method || "—"}
                      </span>
                    </div>
                    <div>
                      <span className="font-medium">Size Plan: </span>
                      <span className="text-muted-foreground">
                        {riskData.execution_ticket?.initial_size_plan || "—"}
                      </span>
                    </div>
                  </div>
                  <div className="space-y-4">
                    <div>
                      ...
                      <span className="text-muted-foreground">
                        {typeof riskData.execution_ticket?.max_slippage_bps ===
                        "number"
                          ? `${riskData.execution_ticket.max_slippage_bps} bps`
                          : "—"}
                      </span>
                    </div>
                    <div>
                      ...
                      <span className="text-muted-foreground">
                        {riskData.execution_ticket?.time_in_force || "—"}
                      </span>
                    </div>
                  </div>
                </div>
            ...
            <Card className="trading-panel-enhanced">
              ...
              <CardContent>
                <div className="space-y-3">
                  {(riskData.what_would_change_my_mind || []).map(
                    (scenario, index) => (
              ...
              <CardContent>
                <div className="space-y-4">
                  <div>
                    ...
                    <span className="text-muted-foreground">
                      {riskData.risk_budget_summary?.exposure_after_trade ||
                        "—"}
                    </span>
                  </div>
                  <div>
                    ...
                    <span className="text-muted-foreground">
                      {riskData.risk_budget_summary?.position_risk || "—"}
                    </span>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {(
                        riskData.risk_budget_summary?.hard_constraints || []
                      ).map((c, i) => (
                        <div
                          key={i}
                          className="p-2 bg-red-50 rounded text-sm font-medium"
                        >
                          {c}
                        </div>
                      ))}
        ...
        {activeTab === "monitoring" && (
          <Card className="trading-panel-enhanced">
            ...
            <CardContent>
              <div className="space-y-3">
                {(riskData.execution_ticket?.monitoring_checklist || []).map(
                  (item, index) => (
                    <div
                      key={index}
